...
! 21.subroutine generateCaE(gCanE1) 
...
real xobs(4) 
  ...
  open (10,file=inputfile)
    ...
    do i=1,ndays
      ...
      rData1.ob(i,1)=max(0.0,rData1.ob(i,1))
    end do
    do j=2,rData1.leadT
      do i=1,ndays-j+1
...
character(c100) asRecord 
  ...
  open(unit=10,file=CanE.CoFile)
    do 
      ...
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.ObFile')) read(tvalue,*) CanE.ObFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.flagPo')) read(tvalue,*) CanE.flagPo
      !if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.PTmin')) read(tvalue,*) CanE.PTmin
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.flagCG')) read(tvalue,*) CanE.flagCG
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.flagT')) read(tvalue,*) CanE.flagT

      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.SiFile')) read(tvalue,*) CanE.SiFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.SiFile1')) read(tvalue,*) CanE.SiFile1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.ReFile')) read(tvalue,*) CanE.ReFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.leadT')) read(tvalue,*) CanE.leadT
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.leadT1')) read(tvalue,*) CanE.leadT1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.nparint')) read(tvalue,*) CanE.nparint
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.x1')) read(tvalue,*) CanE.x1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.x2')) read(tvalue,*) CanE.x2
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.y1')) read(tvalue,*) CanE.y1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.y2')) read(tvalue,*) CanE.y2
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.minwin')) read(tvalue,*) CanE.minwin
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.maxwin')) read(tvalue,*) CanE.maxwin
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.DoFile')) read(tvalue,*) CanE.DoFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.BeginYear ')) read(tvalue,*) CanE.BeginYear 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.EndYear')) read(tvalue,*) CanE.EndYear 

      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.minall')) read(tvalue,*) CanE.minall 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.minpos ')) read(tvalue,*) CanE.minpos 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.pop_fraction ')) read(tvalue,*) CanE.pop_fraction 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.iobstran ')) read(tvalue,*) CanE.iobstran 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.ifcsttran ')) read(tvalue,*) CanE.ifcsttran 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.iopt_rho ')) read(tvalue,*) CanE.iopt_rho 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.cor_weight ')) read(tvalue,*) CanE.cor_weight 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.cavgobsx ')) read(tvalue,*) CanE.cavgobsx 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.cavgfcstx ')) read(tvalue,*) CanE.cavgfcstx 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.verstats ')) read(tvalue,*) CanE.verstats 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.nparint ')) read(tvalue,*) CanE.nparint 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.nems ')) read(tvalue,*) CanE.nems 
    
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.Events')) then
        read(tvalue,*) CanE.Events 
        allocate(CanE.Estart(CanE.Events))
        allocate(CanE.Estop(CanE.Events))
      end if 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.Estart'))read(10,*)(CanE.Estart(i),i=1,CanE.Events)  
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE4.Estop'))read(10,*)(CanE.Estop(i),i=1,CanE.Events)  
...
real(r8)  x1,y1,xx,yy,wx1,wy1,wx2,wy2 
    ...
    open(10,file=CFSData1.WFile)  
      do j=CFSData1.nrows,1,-1
        write(10,'(999(i3,a))')(mask(i,j),char(9),i=1,CFSData1.ncols)
...
real tk
  ...
  !jday1=JULDAY(1,1,epp.BeginYear)  
  write(Tdate,'(i4,2i2.2)') epp.iYear,epp.imonth,epp.iday
  write(outputfile,'(5a)')trim(epp.ReFile),trim(epp.EName),'\',Tdate,'.txt' 
  call Wrecord(outputfile)
  if (epp.flagOs .eq. 2)   call slashchange(outputfile)
  ...
  if(alive) then 
    ...
    open(10,File=outputfile)
    write (10,'(9a,\)')'Lon',char(9),'Lat',char(9),&
	   'LeadT',char(9),'Fcst',char(9),'EnsM'
    do i=1,epp.nems
...
character(c100) asRecord 
  ...
  gCanE1.leadT=epp.leadT
  gCanE1.ndays=epp.ndays
  gCanE1.Events=epp.Events
  gCanE1.Estart=>epp.Estart
  gCanE1.Estop=>epp.Estop
  !gCanE1.ob=>epp.ob
  !gCanE1.Eob=>epp.Eob  ![date, Events]
  !call generateCaE(gCanE1)
  gCanE1.ob=>epp.si
  gCanE1.Eob=>epp.Esi    
  call generateCaE(gCanE1)

  gCanE1.ndays=epp.ndays1
  gCanE1.ob=>epp.ep
  gCanE1.Eob=>epp.Eep    
  call generateCaE(gCanE1)
...
character(c100) asRecord 
  ...
  open(unit=10,file=epp.CoFile)
    do 
      ...
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('epp2.Estart'))read(10,*)(epp.Estart(i),i=1,epp.Events)  
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('epp2.Estop'))read(10,*)(epp.Estop(i),i=1,epp.Events)  
...
character(c100) asRecord 
  ...
  open(unit=10,file=epp.CoFile)
    do 
      ...
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('epp1.Estart'))read(10,*)(epp.Estart(i),i=1,epp.Events)  
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('epp1.Estop'))read(10,*)(epp.Estop(i),i=1,epp.Events)  
...
character(c100) asRecord 
  ...
  open(unit=10,file=CanE.CoFile)
    do 
      read(10,*,end=10)tempc,tvalue        
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.ObFile')) read(tvalue,*) CanE.ObFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.flagPo')) read(tvalue,*) CanE.flagPo
      !if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.PTmin')) read(tvalue,*) CanE.PTmin
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.flagCG')) read(tvalue,*) CanE.flagCG
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.flagT')) read(tvalue,*) CanE.flagT

      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.SiFile')) read(tvalue,*) CanE.SiFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.SiFile1')) read(tvalue,*) CanE.SiFile1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.ReFile')) read(tvalue,*) CanE.ReFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.leadT')) read(tvalue,*) CanE.leadT
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.leadT1')) read(tvalue,*) CanE.leadT1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.nparint')) read(tvalue,*) CanE.nparint
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.x1')) read(tvalue,*) CanE.x1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.x2')) read(tvalue,*) CanE.x2
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.y1')) read(tvalue,*) CanE.y1
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.y2')) read(tvalue,*) CanE.y2
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.minwin')) read(tvalue,*) CanE.minwin
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.maxwin')) read(tvalue,*) CanE.maxwin
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.DoFile')) read(tvalue,*) CanE.DoFile
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.BeginYear ')) read(tvalue,*) CanE.BeginYear 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.EndYear')) read(tvalue,*) CanE.EndYear 

      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.minall')) read(tvalue,*) CanE.minall 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.minpos ')) read(tvalue,*) CanE.minpos 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.pop_fraction ')) read(tvalue,*) CanE.pop_fraction 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.iobstran ')) read(tvalue,*) CanE.iobstran 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.ifcsttran ')) read(tvalue,*) CanE.ifcsttran 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.iopt_rho ')) read(tvalue,*) CanE.iopt_rho 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.cor_weight ')) read(tvalue,*) CanE.cor_weight 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.cavgobsx ')) read(tvalue,*) CanE.cavgobsx 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.cavgfcstx ')) read(tvalue,*) CanE.cavgfcstx 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.verstats ')) read(tvalue,*) CanE.verstats 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.nparint ')) read(tvalue,*) CanE.nparint 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.nems ')) read(tvalue,*) CanE.nems 
    
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.Events')) then
        read(tvalue,*) CanE.Events 
        allocate(CanE.Estart(CanE.Events))
        allocate(CanE.Estop(CanE.Events))
      end if 
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.Estart'))read(10,*)(CanE.Estart(i),i=1,CanE.Events)  
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('CanE3.Estop'))read(10,*)(CanE.Estop(i),i=1,CanE.Events)  
  ...
  do i=1,epp.ndays 
    if (epp.Eens(i,1,1).lt.0) then
      ...
      do j=1,epp.Events  
        ...
        do imem=1,epp.nems  
          ipp=epp.indEep(cday,k,imem)	![365,Events,members]
...
character(c100) asRecord 
  ...
  open(unit=10,file=epp.CoFile)
    do 
      ...
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('epp.Estart'))read(10,*)(epp.Estart(i),i=1,epp.Events)  
      if (tempc(1:1).ne.'$'.and.trim(tempc).eq.trim('epp.Estop'))read(10,*)(epp.Estop(i),i=1,epp.Events)  
...
integer julday,i,j
  ...
  !j=max(i,epp.ndays)
  allocate(epp.ep(epp.ndays1+epp.leadT,epp.leadT))
...
character(c100) asRecord 
  ...
  open (10,file=outputfile) 
      ...
      do i=1,365   
        write (tempc,'(365(a,i3))') (char(9),CanE.nall(i,j),j=1,CanE.Events)   
		call delblank(tempc)
        write (10,'(a)')trim(tempc)
      ...
      do k=1,4
	    write (10,'(a,i1,TR1,i3,TR1,i3)') trim('stdobscal'),k,365,CanE.Events   
        do i=1,365   
          write (tempc,'(365(a,f8.2))') (char(9),CanE.stdobscal(i,j,k),j=1,CanE.Events)   
          call delblank(tempc)
          write (10,'(a)')trim(tempc)
          !write (10,'(20(TR1,f5.2))') (CanE.stdobscal(i,j,k),j=1,CanE.Events)   
        end do 
      end do 
...
integer i,j,k
  ...
  gCanE1.leadT=CanE.leadT
  gCanE1.ndays=CanE.ndays
  gCanE1.Events=CanE.Events
  gCanE1.ob=>CanE.ob
  gCanE1.Estart=>CanE.Estart
  gCanE1.Estop=>CanE.Estop
  gCanE1.Eob=>CanE.Eob
...
integer  i,j,k  
  ...
  do i=1,gCanE1.ndays
    do j=1,gCanE1.Events
      gCanE1.Eob(i,j)=0
      do k=gCanE1.Estart(j),gCanE1.Estop(j)
        gCanE1.Eob(i,j)=gCanE1.Eob(i,j)+gCanE1.ob(i,k)
      end do
      gCanE1.Eob(i,j)=gCanE1.Eob(i,j)/(gCanE1.Estop(j)-gCanE1.Estart(j)+1)
    end do
...
integer julday,jday1,jday2,cday
  ...
  open (10,file=inputfile )
    read (10,*) year1,imonth1,iday1,year2,imonth2,iday2  
    ...
    if (rData1.nparint.eq.0)  then  !GFS data
      ...
      do i=1,rData1.ndays 
        ...
        do j=1,rData1.leadT
          rData1.si(i,j)=rData1.si(i,j)/1.0 !(:,:)
        end do
      end do  
    else   !CFS data    
      if (rData1.nparint.eq.1) then
      ...
      if (rData1.nparint.eq.1) then
        do i=1,rData1.ndays                   
          ...
          do k=1,rData1.leadT
            if (rData1.flagT.eq.0) then
              rData1.si(i,k)=rData1.si(i,k)/1.0  
            else
              rData1.si(i,k)=rData1.si(i,k)/1.0+273.16 !-200 
            end if
          end do     
        end do          
      else
        do i=rData1.BeginYear,rData1.EndYear
          do j=1,365,rData1.nparint
            ...
            do k=1,rData1.leadT
              if (rData1.flagT.eq.0) then
                rData1.si(cday,k)=rData1.si(cday,k)/1.0  
              else
                rData1.si(cday,k)=rData1.si(cday,k)/1.0+273.16 
              end if
            end do     
          end do
        end do
...
integer   months(12),iyear,imonth,iday,ipd  
  ...
  enddo
    !V=考/|米|ㄛwhere 考=﹟﹉(xi-u)^2/(n-1)ㄛu=(﹉xi)/n
  !do iyear=Obsdata1.BeginYear,Obsdata1.EndYear   
  !  j= iyear-Obsdata1.BeginYear+1
  !  if((mod(iyear,4)==0.and.mod(iyear,100)/=0).or. mod(iyear,400)==0)then
  !    months=(/31,29,31,30,31,30,31,31,30,31,30,31/)
